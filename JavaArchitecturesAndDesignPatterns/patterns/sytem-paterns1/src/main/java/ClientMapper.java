...
public class ClientMapper {
    ...
    public Client findById(Long idClient) throws SQLException, RecordNotFoundException {
        ...
        try (ResultSet resultSet = statement.executeQuery()) {
            while (resultSet.next()) {
                return newClientFromResultSet(resultSet);
            }
        }
        throw new RecordNotFoundException(idClient);
    }

    private Client newClientFromResultSet(ResultSet resultSet) throws SQLException {
    ...
    public List<Client> findAllClients() throws SQLException, RecordNotFoundException {
        List<Client> clientList = new ArrayList<>();
        if (!isEmpty()) {
            PreparedStatement statement = connection.prepareStatement(
                    "SELECT `id`, `login`, `password`, `firstName`, `lastName`, `email` FROM `clients`");
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
                    clientList.add(newClientFromResultSet(resultSet));
                }
            }
        }
        return clientList;
    ...
    public boolean isEmpty() throws SQLException {
        PreparedStatement statement = connection.prepareStatement("SELECT count(*) FROM `clients`");
        try (ResultSet resultSet = statement.executeQuery()) {
            while (resultSet.next()) {
                return resultSet.getInt(1) < 1;
            }

...
//        firstEx();
//        System.out.println(countWordsInFile(Paths.get("text.txt")));
//        flatMapEx();
...
//        List<Integer> wordsLength = list.stream().map(strToLen).collect(Collectors.toList());
    ...
    public static void printTheMostFrequentWordInFile(Path path) {
        try {
            Files.lines(path)
                    ...
                    .limit(1)
                    .findFirst().ifPresent(w -> System.out.println("The most frequent word: " + w.getKey()));
        } catch (IOException e) {
    ...
    public static void countExample() {
        Map<Integer, Long> map = Stream.of("A", "BB", "AA", "B", "C", "EE", "NNN", "X", "QQ").collect(Collectors.groupingBy(String::length, Collectors.counting()));
